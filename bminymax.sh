...
minValencia=`cat consumos.txt | grep Valencia | sort -r | head -1 | tail -1`
maxBarcelona=`cat consumos.txt | grep Barcelona | head -1 | tail -1`
minBarcelona=`cat consumos.txt | grep Barcelona | sort -r | head -1 | tail -1`
maxMadrid=`cat consumos.txt | grep Madrid | head -1 | tail -1`
minMadrid=`cat consumos.txt | grep Madrid | sort -r | head -1 | tail -1`
